...
Start with `https://mcp.blocksize.info/quickstart/first-price` to verify a live response, then copy the matching implementation from the public repository. New eligible agent identities receive the standard free allowance of 15,000 live-data credits every month. If the allowance is unavailable or exhausted, the tool returns the original x402 challenge so the caller can pay per call or start a subscription trial.
