...
package. Do not submit it as the `0.5.0` source until the accepted release
...
```text
deliverables/blocksize-market-data-claude-plugin-0.5.0.zip
```
...
- Until GitHub is synced, use
  `deliverables/blocksize-market-data-claude-plugin-0.5.0.zip` only for a
  controlled local validation or an explicit archive-upload workflow. Its
  checksum is recorded in `deliverables/agent-skill-release-0.6.0.json`, whose
