...
Current controlled candidates are OpenAI/Codex `0.4.0`, Claude `0.3.0`, Cursor
`1.3.0`, and standalone skill `0.4.0`. They are local release candidates until
signed-in provider acceptance and public-source provenance pass.
...
The repo marketplace is `.agents/plugins/marketplace.json`. It gives the public
discovery and authenticated live MCP connections distinct identities.
...
The repo marketplace is `.claude-plugin/marketplace.json`; installation remains
blocked until the corrected repository revision is public.
...
The MCP-only deeplink does not install the skill. Full skill discovery requires
the complete Cursor plugin package.

## RWA registry boundary

This release does not expose the RWA registry as an authenticated connector
tool. The safeguarded RWA registry and venue pagination remain a separate HTTP
surface and must not be marketed as plugin functionality until the safeguarded
server release passes staging and production promotion.
...
6. Build archives only with `scripts/build_agent_skill_packages.py` and verify
   the checksum manifest before publishing.
